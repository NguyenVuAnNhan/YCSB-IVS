...
$YCSB load jdbc -s -P $WORKLOAD_FILE -P $JDBC_PROPERTIES \
    -p db.url="$DB_URL" -p db.user="$DB_USERNAME" -p db.passwd="$DB_PWD" \
    -p postgres.array.extend=true > $OUTPUT_CSV 
...
$YCSB load jdbc -s -P $WORKLOAD_FILE -P $JDBC_PROPERTIES \
    -p db.url="$UNCHANGE_DB_URL" -p db.user="$DB_USERNAME" -p db.passwd="$DB_PWD" \
    -p postgres.array.extend=true > $OUTPUT_CSV
...
for epoch in $(seq 1 2); do
    for run in $(seq 1 2); do
        ...
        $YCSB run jdbc -s -P $WORKLOAD_FILE -P $JDBC_PROPERTIES \
            -p db.url="$DB_URL" -p db.user="$DB_USERNAME" -p db.passwd="$DB_PWD" \
            -p postgres.array.extend=true -p fieldlengthhistogram="$HISTOGRAM_FILE" > $OUTPUT_CSV 2>&1
        ...
        $YCSB run jdbc -s -P $WORKLOAD_FILE -P $JDBC_PROPERTIES \
            -p db.url="$DB_URL" -p db.user="$DB_USERNAME" -p db.passwd="$DB_PWD" \
            -p postgres.array.extend=true -p fieldlengthhistogram="$HISTOGRAM_FILE" > $OUTPUT_CSV
        cpu=$(ps -u postgres -o %cpu= | awk '{sum += $1} END {print sum}')
        memory=$(ps -u postgres -o %mem= | awk '{sum += $1} END {print sum}')
        collect_postgres_metrics $DB_NAME
        write_result "FALSE"
        ...
        if (( $((10*($epoch-1)+$run)) % 1 == 0 )); then
            ...
            $YCSB run jdbc -s -P $WORKLOAD_FILE -P $JDBC_PROPERTIES \
                -p db.url="$BACKUP_URL" -p db.user="$DB_USERNAME" -p db.passwd="$DB_PWD" \
                -p postgres.array.extend=true -p fieldlengthhistogram="$HISTOGRAM_FILE" > $OUTPUT_CSV
            cpu=$(ps -u postgres -o %cpu= | awk '{sum += $1} END {print sum}')
            memory=$(ps -u postgres -o %mem= | awk '{sum += $1} END {print sum}')
